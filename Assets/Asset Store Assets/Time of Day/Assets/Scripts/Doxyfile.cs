...
- Rotating night sky<br>
- Realistic moon position<br>
- Continuous moon phases<br>
...
Time of Day offers full support for Unity 5 image-based ambient light and reflections. It can update both the per-scene ambient light and the per-scene reflection probe at runtime. Ambient light can be flat, trilight and spherical harmonics. The reflection probe is always a cubemap. Both contain approximations of the atmosphere in the top half and lerp to the configured ambient light color towards the bottom half. This can be looked at as the ground color since Time of Day cannot know what color to use for the ground by itself.

In addition to the per-scene ambient light and the per-scene reflection probe, Unity 5 offers statically baked reflection probes that can be scattered around a scene. Since those are baked in-editor, they do not change over the course of the day / night cycle. It is therefore recommended to only use them inside buildings or in outdoor scenes with somewhat static lighting conditions. Unity automatically falls back to the realtime per-scene ambient light and reflections that are calculated by Time of Day for all objects that do not have baked light probes nearby.
...
It is possible to export custom presets via the "Export" button in the TOD_Sky inspector panel and import them on a different prefab or even in a different project via the "Import" button.
...
- DeviceTime:
  Automatically set the time of day to the device time on scene start
...
<strong>REMARK:</strong>
Always disable the child objects instead of their individual components like mesh renderers. The enabled states of components are being be modified by the sky dome scripts, which will probably override your changes.
